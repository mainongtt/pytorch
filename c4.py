...
data_transform = {x:transforms.Compose([transforms.Scale([224, 224]),
                                       transforms.ToTensor(),
                                        ...
                                        ]) for x in ['train', 'valid']}   # 这一步类似预处理,相比于手动搭建模型的方法，这里增加了预处理，图像大小为resnet50的标准输入
image_datasets = {x:datasets.ImageFolder(root =  '/content/drive/My Drive/DogsVSCats',
                                        transform = data_transform[x]) for x in ['train', 'valid']}  # 这一步相当于读取数据
dataloader = {x:torch.utils.data.DataLoader(dataset = image_datasets[x],
                                           batch_size = 16,
                                           shuffle = True) for x in ['train', 'valid']}   # 读取完数据后，对数据进行装载
model = models.resnet50(pretrained=True)   # 调用resnet50的预训练模型参数
print(model)
for parma in model.parameters():
    parma.requires_grad = False     # 冻结全部的梯度，使之梯度不进行更新

# 重新定义全连接层为我们想要的输出维度，进行二分类， 这里第一个全连接层的输入不应该对应是25088么？25088会报错？？修改后2048可以运行
# 这是因为开始的时候把图像裁剪到64*64导致的，标准resnet的输入是224*224的
